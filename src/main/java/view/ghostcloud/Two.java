...
//                        System.out.println("收费通道"+" --> "+lane.getName()+"-"+lane.getType()+" 结束");
    ...
    public static void pushCar(Car car) {
        ...
        lane.addCar(car);
        //因为不会绘图 故在最后输出一下车辆的类型
        System.out.println(car.getName() + " --> " + lane.getName() + "-" + lane.getType() + ", arrive at: " +
                car.getArriveTime() + (car.getType() == 0 ? " ETC车" : " 人工车"));
    }
