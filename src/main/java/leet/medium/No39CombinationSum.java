...
public class No39CombinationSum {
    ...
    private static List<List<Integer>> combinationSum(int[] candidates, int target) {
        ...
        recursionSum2(candidates, target, 0, path, result);
        return result;
    }
    ...
    public static void recursionSum2(int[] candidates, int target, int s, ArrayList<Integer> curr, List<List<Integer>> ans) {
        if (target < 0) {
            return;
        } else if (target == 0) {
        ...
        for (int i = s; i < candidates.length; i++) {
            ...
            recursionSum2(candidates, target - candidates[i], i, curr, ans);
